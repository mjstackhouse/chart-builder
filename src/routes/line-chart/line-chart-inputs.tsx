...
export default function LineChartInputs() {
            ...
            <div className='basis-full items-center pt-3 mb-3' key={`category-${index + 1}-container`}>
              ...
              {[...Array(Number(selections?.datasetsNum))].map((_, index2) => (
                <div className='basis-full flex flex-wrap mb-6' key={`dataset-${index2 + 1}-category-${index + 1}-container`}>
                  ...
                  <input type='number' min={0} id={`dataset-${index2 + 1}-category-${index + 1}-value`} name={`dataset-${index2 + 1}-category-${index + 1}-value`} value={ selections?.datasets?.[index2]?.data?.[index] ? selections.datasets[index2].data[index] : '' } onChange={(e) => handleChange('data', e.target.value, index2, index)} className='basis-full rounded-full border-black border-solid border-2 px-2 py-[0.25rem]'/>
                </div>
              ))}
            </div>
          ))}
