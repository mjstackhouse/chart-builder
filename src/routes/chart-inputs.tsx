    ...
    <div className='flex flex-wrap h-[600px]'>
      <div className='basis-full flex flex-wrap'>
        ...
        <form className='basis-full flex flex-wrap' onSubmit={handleSubmit}>
          <div id='inputs-container' className='basis-full h-[505px] overflow-y-auto pr-3'>
            { 
              selections?.type === 'bar' || selections?.type === 'line' ?
              <div className='basis-full h-full divide-solid divide-y'>
                ...
                <div className='basis-full items-center pt-3 mb-6' key={`category-${index + 1}-container`}>
                  ...
                  <div className='basis-full flex flex-wrap mb-3'>
                    ...
                    <input type='text' id={`category-${index + 1}-name`} name={`category-${index + 1}-name`} value={selections?.dataLabels ? selections.dataLabels[index] : '' } onChange={(e) =>handleChange('dataLabels', e.target.value, index)} required/>
                  </div>
                ...
                {[...Array(Number(selections?.datasetsNum))].map((_, index, arr) => (
                  <div className={`basis-full flex flex-wrap pt-3 ${index === arr.length - 1 ? 'mb-6' : 'mb-3'}`} key={`dataset-${index + 1}-container`}>
                    ...
                    <input type='text' id={`dataset-${index + 1}-label`} name={`dataset-${index + 1}-label`}  value={selections?.datasets ? selections.datasets[index]?.dataProps?.dataLabel : '' } onChange={(e) => handleChange('dataLabel', e.target.value, index)} className='mb-3' required/>
                    <label htmlFor={`dataset-${index + 1}-color`} className='pr-2 basis-full mb-1.5'>Dataset {index + 1} color</label>
                    <input type='color' id={`dataset-${index + 1}-color`} name={`dataset-${index + 1}-color`} value={selections?.datasets ? selections.datasets[index]?.dataProps?.dataColor?.[0] : '' } onChange={(e) => handleChange('dataColor', e.target.value, index)} className='w-12 h-12 border-black border-solid border p-0.5 mb-3' required/>
                  </div>
                ))}
              </div>
              :
                  ...
                  <div className='basis-full items-center pt-3 mb-6' key={`category-${index + 1}-container`}>
                    ...
                    {[...Array(Number(selections?.datasetsNum))].map((_, index2, arr) => (
                      <div className={`basis-full flex flex-wrap ${index2 === arr.length - 1 ? 'mb-6' : 'mb-3'}`} key={`dataset-${index2 + 1}-category-${index + 1}-container`}>
                        <label htmlFor={`dataset-${index2 + 1}-category-${index + 1}-value`} className='pr-2 basis-full mb-1.5'>Dataset {index2 + 1} value</label>
                        <input type='number' min={0} id={`dataset-${index2 + 1}-category-${index + 1}-value`} name={`dataset-${index2 + 1}-category-${index + 1}-value`} value={ selections?.datasets?.[index2]?.data?.[index] ? selections.datasets[index2].data[index] : '' } onChange={(e) => handleChange('data', e.target.value, index2, index)} required/>
                        <label htmlFor={`dataset-${index2 + 1}-category-${index + 1}-color`} className='pr-2 basis-full mb-1.5'>Dataset {index2 + 1} color</label>
                        <input type='color' id={`dataset-${index2 + 1}-category-${index + 1}-color`} name={`dataset-${index2 + 1}-category-${index + 1}-color`} value={selections?.datasets ? selections.datasets[index2]?.dataProps?.dataColor?.[index] : '' } onChange={(e) => handleChange('dataColor', e.target.value, index2, index)} className='w-12 h-12 border-black border-solid border p-0.5' required/>
                      </div>
                    ))}
                  </div>
                ))}
