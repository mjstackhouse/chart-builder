...
export default function PieChartInputs() {
  ...
  function handleChange(type: string, value: string, datasetIndex: number, dataValueIndex?: number) {
    if (type === 'data') {
      let currentDatasets: Dataset[] = [...selections?.datasets!];
      let dataset;

      if (currentDatasets[datasetIndex]) dataset = currentDatasets?.[datasetIndex];
      ...
      let currentDataLabels: string[];

      currentDataLabels = [...selections?.dataLabels!];
      ...
      currentDatasets = [...selections?.datasets!];

      let dataset = currentDatasets?.[datasetIndex];

      if (dataset) {
        if (dataset?.dataProps) {
          if (dataset?.dataProps?.dataColor && dataValueIndex !== undefined) {
            dataset.dataProps.dataColor[dataValueIndex] = value;
          }
        }
      }
...
//   function handleSubmit(e: FormEvent) {
//     e.preventDefault();
//     navigate(`/type/pie/preview?${Object.keys(navLinkParams).map((key) => [key, navLinkParams[key]]).map((param) => `${param[0]}=${encodeURIComponent(param[1] as string)}`).join('&')}`);
//   }
...
//                 <label htmlFor={`category-${index + 1}-color`} className='pr-2 basis-full mb-1.5'>Section color</label>
